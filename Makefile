...
nextcloud_install_directory=/projects/nextcloud_server/apps/documentmanager
...
source:
	...
	#touch $(source_package_name).tar.gz
	#tar --exclude=$(source_package_name).tar.gz -cvzf $(source_package_name).tar.gz ../$(app_name) \
	#--exclude-vcs \
...
test-deploy:
	...
	phpunit -c $(nextcloud_install_directory)/phpunit.xml
